...
const filteredMeds = isMedInList.filter((medObj) => medObj.name.toLowerCase().includes(search.toLowerCase()))
  ...
  <NavBar/> 
      ...
      <Switch> 
          ...
          <Row xs={3} md={3} className="g-4"> {myMeds.map(userMed => <MyMeds key={userMed.id} userMed={userMed} addMedToList={addMedToList} removeMedFromList={removeMedFromList} setMedNotes={setMedNotes} medNotes={medNotes} setMyMeds={setMyMeds}/>)}</Row>
          </Route>
          {/* <Route path="/user_medications/">
